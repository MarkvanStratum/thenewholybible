...
/* ========================================
   STRIPE ACCOUNT #1 (S1) ENDPOINTS
======================================== */

// Endpoint for $35.95
app.post("/api/stripe/s1/one-time-35-95", async (req, res) => {
  try {
    // Correctly initialize the S8 instance using the constructor
    const stripeS8 = new Stripe(process.env.STRIPE_S1_SECRET_KEY);
...
// Endpoint for $43.95
app.post("/api/stripe/s1/one-time-43-95", async (req, res) => {
  try {
    const stripeS8 = new Stripe(process.env.STRIPE_S1_SECRET_KEY);
    const { paymentMethodId, billingDetails } = req.body;
...
// Endpoint for $49.95
app.post("/api/stripe/s1/one-time-59-95", async (req, res) => {
  try {
    const stripeS8 = new Stripe(process.env.STRIPE_S1_SECRET_KEY);
    const { paymentMethodId, billingDetails } = req.body;
