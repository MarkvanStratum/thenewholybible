...
/* ========================================
   STRIPE ACCOUNT #5 (S5) ENDPOINTS
======================================== */

// Endpoint for $29.95
app.post("/api/stripe/s5/one-time-29-95", async (req, res) => {
  try {
    const stripeS5 = new Stripe(process.env.STRIPE_S5_SECRET_KEY);
    const { paymentMethodId, billingDetails } = req.body;
    ...
    const paymentIntent = await stripeS5.paymentIntents.create({
      amount: 2995, // $29.95 in cents
      currency: "usd",
      payment_method: paymentMethodId,
      confirmation_method: "automatic",
    ...
    console.error("Stripe S5 29.95 error:", err);
    res.status(500).json({ error: err.message || "Payment failed" });
  }
});

// Endpoint for $35.95
app.post("/api/stripe/s5/one-time-35-95", async (req, res) => {
  try {
    const stripeS5 = new Stripe(process.env.STRIPE_S5_SECRET_KEY);
    const { paymentMethodId, billingDetails } = req.body;

    const paymentIntent = await stripeS5.paymentIntents.create({
      amount: 3595, // $35.95 in cents
      currency: "usd",
      payment_method: paymentMethodId,
      confirmation_method: "automatic",
      confirm: false,
    ...
    console.error("Stripe S5 35.95 error:", err);
    res.status(500).json({ error: err.message || "Payment failed" });
  }
});

// Endpoint for $49.95
app.post("/api/stripe/s5/one-time-49-95", async (req, res) => {
  try {
    const stripeS5 = new Stripe(process.env.STRIPE_S5_SECRET_KEY);
    const { paymentMethodId, billingDetails } = req.body;
    ...
    console.error("Stripe S5 49.95 error:", err);
    res.status(500).json({ error: err.message || "Payment failed" });
  }
});
